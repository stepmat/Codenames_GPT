...
class AICodemaster(Codemaster):
    ...
    def __init__(self, team="Red", threshold=0.7, brown_ic=None, glove_vecs=None, word_vectors=None):
        super().__init__()
        self.name = "w2v"
        self.team = team
    ...
    def get_clue(self):
        ...
        for i in range(25):
            if self.words[i][0] == '*':
                continue
            elif self.maps[i] != self.team:
                bad_words.append(self.words[i].lower())
            else:
                red_words.append(self.words[i].lower())
        print("TEAM WORDS:\t", red_words)
        ...
        for clue_num, clue in bests.items():
            ...
            for word in best_red_word:
                ...
                if dist < best:
                    best = dist
            if worst < self.threshold and worst != -np.inf:
